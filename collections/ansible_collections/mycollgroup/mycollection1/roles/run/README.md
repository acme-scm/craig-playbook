Mycollgroup.Mycollection1 Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: mycollgroup.mycollection1.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from mycollgroup.mycollection1
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: mycollgroup.mycollection1.run
      vars:
        run_x: 42
```
